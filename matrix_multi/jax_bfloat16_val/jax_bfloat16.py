...
def main():
    ...
    sssize = 17
    for n in range(3,sssize):
        for m in range(3,sssize):
            for k in range(3,sssize):
                ...
                mat_a = jax.random.normal(key, (size_n, size_m),dtype=jnp.bfloat16)
                mat_b = jax.random.normal(key, (size_m, size_k),dtype=jnp.bfloat16)
